	...
	<script type="text/javascript">
		...
		function build_emps_table(result){
			...
			$.each(emps,function(index,item){
				...
				var genderR = item.gender == 'M' ?"男":"女";
				var gender = $("<td></td>").append(genderR);
				...
				*/
				...
				//未删除按钮添加一个自定义的属性来表示当前删除的员工id
				delBtn.attr("delete-id",item.empId);
				var btn = $("<td></td>").append(editBtn).append(" ").append(delBtn);
				//append方法执行结束后还是返回原来的元素，所以可以执行链式操作
		...
		function build_page_nav(result){
			...
			lastPageLi.click(function(){
				to_page(to_page(result.extend.pageInfo.pages));
			});
			//添加首页和前一页
